...
if luci.sys.call("pidof koolproxy >/dev/null") == 0 then
	...
	status = translate("<strong><font color=\"red\">KoolProxy  已关闭</font></strong>")
end

o = Map("koolproxy", translate("KoolProxy"), translate("使用HTTPS过滤模式时需要为客户端安装证书并信任。证书下载方法：在客户端使用浏览器打开110.110.110.110） <br /><font color=\"red\"><br /></font>"))
...
e=t:taboption("base",Flag,"koolproxy_host",translate("Adblock Plus Host 规则"))
e.default=0
e:depends("koolproxy_mode","2")


e = t:taboption("base", ListValue, "koolproxy_acl_default", translate("过滤模式"))
e.default = 1
e.rmempty = false
e:value(0, translate("不过滤"))
e:value(1, translate("HTTP过滤模式"))
e:value(2, translate("HTTP + HTTPS过滤模式"))
e:value(3, translate("全端口模式"))
e.description = translate(string.format("<font color=\"blue\"><strong>可在访问控制中为指定设备独立设置过滤模式</strong></font>"))
...
e.description = translate("这些已经加入的网站不会使用广告过滤器。请输入网站的域名，如：google.com，每个地址一行")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的网址强制使用广告过滤器。请输入网站的域名，如：google.com，每个地址一行")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已加入的ip地址不使用广告过滤器。请输入ip地址段或ip地址，每个地址一行。例如，112.123.134.145 / 24或112.123.134.145")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的ip地址强制使用广告过滤器，请输入ip地址段或ip地址，每个地址一行。例如，112.123.134.145 / 24或112.123.134.145")
e.rows = 28
e.wrap = "off"
e.rmempty = false
