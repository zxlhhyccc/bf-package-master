...
m:section(SimpleSection).template = "nps-server/nps-server_status"

s = m:section(TypedSection, "nps-server")
s.addremove = false
s.anonymous = true

s:tab("setting", translate("Basic Setting"))
s:tab("web", translate("Web Setting"))
s:tab("proxy", translate("NpsProxy"))
s:tab("bridge", translate("Bridge"))
s:tab("key", translate("Auth Key"))

enable = s:taboption("setting", Flag,"enabled", translate("Enabled"))
...
log_path = s:taboption("setting", Value,"log_path", translate("Log Path"))
log_path.datatype = "string"
log_path.default = "nps-server.log"
log_path.optional = false
log_path.rmempty = false

runmode = s:taboption("setting", ListValue, "runmode", translate("Boot mode"))
runmode.default = "dev"
runmode:value("dev", translate("Dev"))
runmode:value("pro", translate("Pro"))

ip_limit = s:taboption("setting", ListValue, "ip_limit", translate("ip_limit"), translate("ip limit switch"))
