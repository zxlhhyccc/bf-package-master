...
add() {
	local FLAG SMARTDNS_CONF LOCAL_GROUP REMOTE_GROUP REMOTE_PROXY_SERVER TUN_DNS_PROTO TUN_DNS TCP_NODE PROXY_MODE NO_PROXY_IPV6 NO_LOGIC_LOG NFTFLAG
	eval_set_val $@
	lua $APP_PATH/helper_smartdns_add.lua -FLAG $FLAG -SMARTDNS_CONF $SMARTDNS_CONF -LOCAL_GROUP ${LOCAL_GROUP:-nil} -REMOTE_GROUP ${REMOTE_GROUP:-nil} -REMOTE_PROXY_SERVER ${REMOTE_PROXY_SERVER} -TUN_DNS_PROTO $TUN_DNS_PROTO -TUN_DNS $TUN_DNS -TCP_NODE $TCP_NODE -PROXY_MODE $PROXY_MODE -NO_PROXY_IPV6 ${NO_PROXY_IPV6:-0} -NO_LOGIC_LOG ${NO_LOGIC_LOG:-0} -NFTFLAG ${NFTFLAG:-0}
}
