...
function update() {
	...
	sha="$(curl -sL https://github.com/$repo/releases/download/v$tag/$type-source-v$tag.tar.gz | sha256sum | awk '{print $1}')"
	[ -n "$sha" ] || return 1
...
update "syncthing" "syncthing/syncthing" "syncthing-"$(awk -F "PKG_VERSION:=" '{print $2}' "$CURDIR/Makefile" | xargs)"" 
