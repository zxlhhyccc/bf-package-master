...
float = s:taboption("basic",Flag,"float", translate("Float"))
float.description = translate("Allow the remote host to change its IP address or port,UDP mode needs to enable this option.")
...
username_as_common_name:depends("auth_user_pass_verify", "/etc/checkpsw.sh via-env")
...
verify_client_cert:depends("auth_user_pass_verify", "/etc/checkpsw.sh via-env")
