...
function index()
	if not nixio.fs.access("/etc/config/luci_socat") then
		return
	end
...
function act_status()
	...
	e.status = luci.sys.call(string.format("busybox ps -w | grep -v 'grep' | grep '/var/etc/luci_socat/%s' >/dev/null", luci.http.formvalue("id"))) == 0
