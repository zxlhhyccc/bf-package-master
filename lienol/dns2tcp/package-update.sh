#!/bin/bash
# 自动更新 dns2tcp 版本、commit 并计算 HASH
...
REPO="https://github.com/zfl9/dns2tcp"
REPO_API="https://api.github.com/repos/zfl9/dns2tcp/releases/latest"
...
if [ "$VER" != "$OLD_VER" ] || [ "$COMMIT" != "$OLD_COMMIT" ]; then
    ...
    # 删除旧源码包和哈希
    rm -f dl/dns2tcp-${OLD_VER}.tar.zst

    # 清理旧缓存（触发重新编译）
    make package/dns2tcp/clean V=s
    ...
    # 重新下载源码包
    make package/dns2tcp/download V=s

    # 重新生成校验和
    TARFILE="dl/dns2tcp-${VER}.tar.zst"
    if [ -f "$TARFILE" ]; then
