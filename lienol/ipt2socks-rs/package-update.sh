#!/bin/bash
# 自动更新 ipt2socks-rs 版本、commit 并计算 HASH
...
REPO="https://github.com/fokx/ipt2socks-rs"
REPO_API="https://api.github.com/repos/fokx/ipt2socks-rs/releases/latest"
...
if [ "$VER" != "$OLD_VER" ] || [ "$COMMIT" != "$OLD_COMMIT" ]; then
    ...
    # 清理旧缓存（触发重新编译）
    make package/ipt2socks/clean V=s
    ...
    # 重新下载源码包
    make package/ipt2socks-rs/download V=s

    # 重新生成校验和
    TARFILE="dl/ipt2socks-${VER}.tar.gz"
    if [ -f "$TARFILE" ]; then
