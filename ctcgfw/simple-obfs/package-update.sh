#!/bin/bash
# 自动更新 simple-obfs 版本、commit 并计算 HASH
...
REPO_API="https://api.github.com/repos/shadowsocks/simple-obfs/releases/latest"
...
if [ "$VER" != "$OLD_VER" ] || [ "$COMMIT" != "$OLD_COMMIT" ]; then
    ...
    # 删除旧源码包和哈希
    rm -f dl/simple-obfs-${OLD_VER}.tar.zst

    # 清理旧缓存（触发重新编译）
    make package/simple-obfs/clean V=s
    ...
    # 重新下载源码包
    make package/simple-obfs/download V=s

    # 重新生成校验和
    TARFILE="dl/simple-obfs-${VER}.tar.zst"
    if [ -f "$TARFILE" ]; then
