#!/bin/bash
# 自动更新 ssocks 版本、commit 并计算 HASH
...
REPO="https://github.com/tostercx/ssocks"
#REPO_API="https://api.github.com/repos/tostercx/ssocks/releases/latest"
...
API_DATA=$(curl -s https://api.github.com/repos/tostercx/ssocks/commits \
...
if [ "$API_DATA" != "$OLD_DATA" ] || \
    ...
    # 删除旧源码包和哈希
    rm -f dl/ssocks-${OLD_VER}.tar.zst

    # 清理旧缓存（触发重新编译）
    make package/ssocks/clean V=s
    ...
    # 重新下载源码包
    make package/ssocks/download V=s

    # 重新生成校验和
    TARFILE="dl/ssocks-${OLD_VER}.tar.zst"
    if [ -f "$TARFILE" ]; then
