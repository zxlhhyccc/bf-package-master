...
function update() {
	...
	tag="$(curl -H "Authorization: $GITHUB_TOKEN" -sL "https://api.github.com/repos/$repo/releases/latest" | jq -r ".tag_name")"
	[ -n "$tag" ] && {
	sha="$(curl -sL https://github.com/$repo/releases/download/$tag/chinadns-ng@${type}@${res}@fast+lto | shasum -a 256 | awk '{print $1}')"
	...
	line="$(awk "/PKG_SOURCE:=chinadns-ng@${type}@${res}@fast\+lto/ {print NR}" "$CURDIR/Makefile")"
...
function _update() {
	...
	tag="$(curl -H "Authorization: $GITHUB_TOKEN" -sL "https://api.github.com/repos/$repo/releases/latest" | jq -r ".tag_name")"
	[ -n "$tag" ] && {
	sha="$(curl -sL https://github.com/$repo/releases/download/$tag/chinadns-ng@${type}@${res}+${reg}@fast+lto | shasum -a 256 | awk '{print $1}')"
	...
	line="$(awk "/PKG_SOURCE:=chinadns-ng@${type}@${res}\+${reg}@fast\+lto/ {print NR}" "$CURDIR/Makefile")"
...
_update "arm-linux-musleabi" "zfl9/chinadns-ng" "generic" "v7a"
