...
function update() {
	...
	old_hash="$(awk -F "PKG_MIRROR_HASH:=" '{print $2}' "$CURDIR/Makefile" | xargs)"
...
update "sing-box" "rustdesk/rustdesk-server" "rustdesk-server-"$(awk -F "PKG_VERSION:=" '{print $2}' "$CURDIR/Makefile" | xargs)"" 
