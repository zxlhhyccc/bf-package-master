#!/bin/bash
# 自动更新 mosdns 版本、commit 并计算 HASH
...
REPO="https://github.com/IrineSistiana/mosdns"
REPO_API="https://api.github.com/repos/IrineSistiana/mosdns/releases/latest"
...
if [ "$VER" != "$OLD_VER" ] || \
    ...
    # 删除旧源码包和哈希
    rm -f dl/mosdns-${OLD_VER}.tar.gz

    # 清理旧缓存（触发重新编译）
    make package/mosdns/clean V=s
