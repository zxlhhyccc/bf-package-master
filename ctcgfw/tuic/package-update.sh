#!/bin/bash
# 自动更新 shadowsocks-rust 版本、commit 并计算 HASH
...
REPO="https://github.com/Itsusinn/tuic.git"
REPO_API="https://api.github.com/repos/Itsusinn/tuic/releases/latest"
...
if [ "$VER" != "$OLD_VER" ] || [ "$COMMIT" != "$OLD_COMMIT" ]; then
    ...
    # 删除旧源码包和哈希
    rm -f dl/tuic-${OLD_VER}.tar.gz

    # 清理旧缓存（触发重新编译）
    make package/tuic/clean V=s
    ...
    # 重新下载源码包
    make package/tuic/download V=s

    # 重新生成校验和
    TARFILE="dl/tuic-${VER}.tar.gz"
    if [ -f "$TARFILE" ]; then
