...
REPO_API="https://api.github.com/repos/txthinking/brook/releases/latest"
...
API_DATA="$(curl -s https://api.github.com/repos/txthinking/brook/commits \
...
if [ "$VER" != "$OLD_VER" ] || \
    ...
    # 删除旧源码包和哈希
    rm -f dl/brook-${OLD_VER}.tar.gz

    # 清理旧缓存（触发重新编译）
    make package/brook/clean V=s
    ...
    # 重新下载源码包
    make package/brook/download V=s

    # 重新生成校验和
    TARFILE="dl/brook-${VER}.tar.gz"
    if [ -f "$TARFILE" ]; then
