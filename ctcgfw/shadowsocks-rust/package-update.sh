...
if [ "$VER" != "$OLD_VER" ] || [ "$COMMIT" != "$OLD_COMMIT" ]; then
    ...
    # 删除旧源码包和哈希
    rm -f dl/shadowsocks-rust-${OLD_VER}.tar.gz

    # 清理旧缓存（触发重新编译）
    make package/shadowsocks-rust/clean V=s
    ...
    # 重新生成校验和
    TARFILE="dl/shadowsocks-rust-${VER}.tar.gz"
    if [ -f "$TARFILE" ]; then
