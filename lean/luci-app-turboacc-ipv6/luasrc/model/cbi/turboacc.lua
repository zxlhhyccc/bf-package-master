...
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/xt_FLOWOFFLOAD.ko") then
...
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/mtkhnat.ko") then
...
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/shortcut-fe-cm.ko")
or nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/fast-classifier.ko")
then
...
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/xt_FULLCONENAT.ko") then
fullcone_nat = s:option(Flag, "fullcone_nat", translate("IPv4 FullCone NAT"))
fullcone_nat.default = 0
fullcone_nat.description = translate("Using FullCone NAT can improve gaming performance effectively")
fullcone_nat6 = s:option(Flag, "fullcone_nat6", translate("IPv6 FullCone NAT"))
fullcone_nat6.default = 0
fullcone_nat6.description = translate("Enable extra IPv6 FullCone NAT, most IPv6 traffic do NOT need this")
end

bbr_cca = s:option(ListValue, "bbr_cca", translate("BBR CCA"))
bbr_cca:value("0", translate("CUBIC"))
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/tcp_bbr.ko") then
bbr_cca:value("bbr", translate("Enable BBR"))
end
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/tcp_bbrplus.ko") then
bbr_cca:value("bbrplus", translate("Enable BBRPLUS"))
end
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/tcp_nanqinlang.ko") then
bbr_cca:value("nanqinlang", translate("Enable NANQINLANG"))
end
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/tcp_tsunami.ko") then
bbr_cca:value("tsunami", translate("Enable TSUNAMI"))
end
if nixio.fs.access("/lib/modules/" .. boardinfo.kernel .. "/tcp_tsunamio.ko") then
