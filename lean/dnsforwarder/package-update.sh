#!/bin/bash
# 自动更新 dnsforwarder 版本、commit 并计算 HASH
...
REPO="https://github.com/lifenjoiner/dnsforwarder"
REPO_API="https://api.github.com/repos/lifenjoiner/dnsforwarder/releases/latest"

# 获取新 TAG、COMMIT 等
TAG="$(curl -H "Authorization: $GITHUB_TOKEN" -sL "$REPO_API" | jq -r ".tag_name")"
VER="${TAG#v}"  # TAG 形如 v1.8.11

API_DATA="$(curl -s https://api.github.com/repos/lifenjoiner/dnsforwarder/commits \
...
if [ "$VER" != "$OLD_VER" ] || \
    ...
    # 删除旧源码包和哈希
    rm -f dl/dnsforwarder-${OLD_VER}.tar.zst
    ...
    # 重新下载源码包
    make package/dnsforwarder/download V=s
