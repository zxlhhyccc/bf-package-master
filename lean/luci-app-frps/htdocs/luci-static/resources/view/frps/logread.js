...
function pollLog(e) {
	return Promise.all([
			fs.exec_direct('/usr/libexec/frps-call', [ 'tail' ]).then(function(res) {
				return res.trim()
			}),
			fs.exec_direct('/sbin/logread', [ '-e', 'frps' ]).then(function(res) {
				return res.trim()
			})
		]).then(function(data) {
...
return view.extend({
	render: function() {
		...
		return E([
			E('style', [ css ]),
			E('div', {'class': 'cbi-map'}, [
				E('h2', {'name': 'content'}, '%s - %s'.format(_('frps'), _('Log Data'))),
