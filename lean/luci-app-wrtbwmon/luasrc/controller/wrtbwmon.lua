...
function index()
    entry({"admin", "status", "usage"}, alias("admin", "status", "usage", "details"), _("实时流量统计"), 60)
    entry({"admin", "status", "usage", "details"}, template("wrtbwmon"), _("详细信息"), 10).leaf=true
    entry({"admin", "status", "usage", "config"}, cbi("wrtbwmon/config"), _("配置"), 20).leaf=true
    entry({"admin", "status", "usage", "custom"}, form("wrtbwmon/custom"), _("自定义主机信息"), 30).leaf=true
