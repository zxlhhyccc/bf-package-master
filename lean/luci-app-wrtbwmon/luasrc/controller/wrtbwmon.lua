...
function index()
    entry({"admin", "network", "usage"}, alias("admin", "network", "usage", "details"), _("实时流量统计"), 60)
    entry({"admin", "network", "usage", "details"}, template("wrtbwmon"), _("详细信息"), 10).leaf = true
    entry({"admin", "network", "usage", "config"}, cbi("wrtbwmon/config"), _("配置"), 20).leaf = true
    entry({"admin", "network", "usage", "custom"}, form("wrtbwmon/custom"), _("自定义主机信息"), 30).leaf = true
