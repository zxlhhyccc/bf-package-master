...
function check_latest_version(){
	latest_ver="$(uclient-fetch --no-check-certificate -O- https://github.com/server/UnblockNeteaseMusic/commits/enhanced |tr -d '\n' |grep -Eo 'commit\/[0-9a-z]+' |sed -n 1p |sed 's#commit/##g')"
...
function update_core(){
	...
	uclient-fetch --no-check-certificate -T 10 -O  /tmp/unblockneteasemusic/core/core.tar.gz "https://github.com/server/UnblockNeteaseMusic/archive/enhanced.tar.gz"  >/dev/null 2>&1
